...
**目標:公司內部的開發者程式分享平台——「簡易型 GitHub」。**
開發者登入後建立專案,在專案內上傳**原始碼、執行檔、程式文件**,以版本為單位發布,他人可瀏覽、搜尋、下載。
...
- **MVP 不做**:git 協定(clone/push)、PR / code review、issue、線上編輯、CI 建置、外網公開。
  界線一句話——做的是「有版本管理的程式發布倉庫」,不是分散式版本控制系統。
- 設計細節(領域模型、API、儲存、上傳安全)見 `DOCS/設計_MVP.md`。
