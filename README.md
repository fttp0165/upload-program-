...
側欄、建立版本頁這些畫面只在「已開通登入者」的版型出現,而本專案**不自建帳號系統**
——本機沒有 IdP 就登不進去。`tools/devserver.py`(T92)用測試已經在用的替身
(`FakeOidc` / `FakeStorage`)+ SQLite 檔把真的 app 跑起來,並模擬 gateway 剝掉
`/upload/` 前綴:

```bash
DEV_UNSAFE_LOCAL=1 .venv/bin/python tools/devserver.py    # 綁 127.0.0.1:8080
```

| 開哪個網址 | 看什麼 |
|---|---|
| `http://127.0.0.1:8080/dev/login` | 先開這個:種一個「已登入(管理員)」的 session |
| `http://127.0.0.1:8080/upload/` | 登入後版型 + 側欄(入口連結在最下面) |
| `http://127.0.0.1:8080/upload/projects/demo-tool/releases/new` | 建立版本頁的既有版本號清單 |
| `http://127.0.0.1:8080/upload/help` | 使用教學的三張圖 |

🔴 **這支腳本會偽造 session,只供本機。** 三層結構擋住它進正式環境:
沒有 `DEV_UNSAFE_LOCAL=1` 就拒絕啟動、只綁 `127.0.0.1`、
而 `Dockerfile` 只 `COPY app/` 與 `alembic/`——**image 裡根本沒有 `tools/` 與 `tests/`**。
四條都由 `tests/test_devserver.py` 反向驗證。
