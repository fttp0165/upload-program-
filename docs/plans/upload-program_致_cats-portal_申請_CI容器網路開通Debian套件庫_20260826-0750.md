...
| 2026-08-12(T117) | base image 的 `util-linux` 家族 CVE | 加 `--pull`;當時**運氣好**,上游已重建 |
...
| **`.trivyignore` 記下此 CVE** | 🔴 我方已就同類情境立過**兩次**界線(T82、T117)並拒絕。本服務**散布可執行檔**給公司同仁,掃描是自我加嚴的一環,調鬆掃描是拿紅線換方便。**即使**本 CVE 幾乎確定不適用(見 §5),「我判斷用不到」不是關掉掃描的理由。 |
