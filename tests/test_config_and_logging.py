...
def test_預設單檔上限為500MB():
    """T140(Benny 2026-09-08 改為 500 MB;同日 T138 才剛從 100 MB 調到 200 MB)。

    ⚠ **本斷言換了值,不是放水**(CI 紅線要求說明):原本釘的是 Q7 裁示的
    「50~100M 取上限」,而 2026-09-08 Benny 先選 **200 MB**、同日再改為 **500 MB**。
    釘住的意圖沒變 —— **預設值必須是被裁示過的那個數字**,而不是誰改程式時
    順手寫的數字。
    """
    assert _settings().max_artifact_bytes == 500 * MB
